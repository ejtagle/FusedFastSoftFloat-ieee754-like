...
static constexpr int memreq = arsize2d*sizeof(REAL) + (long)arsize*sizeof(REAL) + (long)arsize*sizeof(int);
static char pool[memreq];

static void *mempool = &pool[0];

extern "C" void linpack(void)
{
    ...
    *norma = 0.0f;
    for (j = 0; j < n; j++)
        for (i = 0; i < n; i++)
            ...
            a[lda*j+i] = (init - REAL(32768.0f))/REAL(16384.0f);
