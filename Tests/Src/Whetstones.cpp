...
#define SIN(x) (SoftFloat)sin(x)
#define COS(x) (SoftFloat)cos(x)
#define TAN(x) tan(x)
#define ATAN(x) (SoftFloat)atan(x)
#define ATAN2(x, y) (SoftFloat)atan2(x, y)
...
static float loop_time[9];
static float loop_mops[9];
static float loop_mflops[9];
...
extern "C" void test_whetstones(void)
{
 ...
 while (count > 0);
	...
	if (TimeUsed > 0) xtra = (long)((float)(duration * xtra) / TimeUsed);
	if (xtra < 1) xtra = 1;
...
void whetstones(long xtra, long x100, int calibrate)
{
	...
	SPDP t = SPDP(0.49999975f);
	SPDP t0 = t;        
	SPDP t1 = SPDP(0.50000025f);
	SPDP t2 = SPDP(2.0f);
	...
	timeres = (dtime() - timea) / (float)(n1mult);
	pout("N1 floating point\0",
		(float)(n1 * 16)*(float)(xtra),
		1,
		e1[3],
		(float)timeres,
		calibrate,
	...
	timeres = (dtime() - timea);
	pout("N2 floating point\0",
		(float)(n2 * 96)*(float)(xtra),
		1,
		e1[3],
		(float)timeres,
		calibrate,
	...
	timeres = (dtime() - timea);
	pout("N3 if then else  \0",
		(float)(n3 * 3)*(float)(xtra),
		2,
		(SPDP)(j),
		(float)timeres,
		calibrate,
	...
	timeres = (dtime() - timea);
	x = e1[0] + e1[1];
	pout("N4 fixed point   \0",
		(float)(n4 * 15)*(float)(xtra),
		2,
		x,
		(float)timeres,
		calibrate,
	...
	timeres = (dtime() - timea);
	pout("N5 sin,cos etc.  \0",
		(float)(n5 * 26)*(float)(xtra),
		2,
		y,
		(float)timeres,
		calibrate,
	...
	timeres = (dtime() - timea);
	pout("N6 floating point\0",
		(float)(n6 * 6)*(float)(xtra),
		1,
		z,
		(float)timeres,
		calibrate,
	...
	timeres = (dtime() - timea);
	pout("N7 assignments   \0",
		(float)(n7 * 3)*(float)(xtra),
		2,
		e1[2],
		(float)timeres,
		calibrate,
	...
	timeres = (dtime() - timea);
	pout("N8 exp,sqrt etc. \0",
		(float)(n8 * 4)*(float)(xtra),
		2,
		x,
		(float)timeres,
		calibrate,
	...
	int section)
{
	...
	if (calibrate == 0)
	{              
		printf("%s %24.17f    ", headings[section], (float)results[section]);    
